...
function parseWinner(scSList) {
  for (const e of (scSList || [])) {
    if (e.Key === 'W') {
      try {
        const v = JSON.parse(e.Value || 'null');
        if (v === 1 || v === '1') return 'Player';
        if (v === 2 || v === '2') return 'Banker';
        if (v === 0 || v === '0') return 'Tie';
      } catch {}
    }
  }
  return null;
...
function parsePhase(scSList) {
  for (const e of (scSList || [])) {
    if (e.Key === 'PN') { try { return JSON.parse(e.Value || 'null'); } catch {} }
  }
  return null;
}

function isGameFinished(game, scSList) {
  if (game.GS === 2 || game.GS === 3) return true;
  for (const e of (scSList || [])) {
    if (e.Key === 'FS') { try { return !!JSON.parse(e.Value || 'false'); } catch {} }
    if (e.Key === 'W')  { try { const v = JSON.parse(e.Value || 'null'); return v !== null; } catch {} }
  }
  return false;
}
